...
module Ancestry
  ...
  class MaterializedPathArray
    ...
    # Arel condition: descendants have child_ancestry as a prefix of their ancestry
    # Slice comparison ensures positional correctness (stale in-memory state safety)
    def self.descendants_condition(attr, child_ancestry, _delimiter)
      ...
      n = child_ancestry.size
      Arel.sql("#{col}[1:#{n}] = ARRAY[#{ids}]::integer[] AND array_length(#{col}, 1) >= #{n}")
    end
